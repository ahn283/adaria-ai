...
- [ ] `src/agent/session.ts` — path change to `~/.adaria/sessions.json` via `paths.ts`
- [ ] `src/agent/memory.ts` — conversation-scoped memory only
- [ ] `src/agent/conversation-summary.ts` — verbatim
- [ ] `src/agent/safety.ts` — pilot-ai ApprovalManager base (growth-agent gates merged in M5)
- [ ] `src/agent/audit.ts` — path change to `~/.adaria/audit.jsonl`
