...
| `mcp-manager.ts` | 🟢 copy | `src/agent/mcp-manager.ts` | MCP server registry / lifecycle. Personal-agent tool servers are dropped but the framework itself powers Mode B (conversational) routing with read-only marketing tools. |
| `mcp-launcher.ts` | 🟢 copy | `src/agent/mcp-launcher.ts` | Spawns MCP servers for Claude CLI. Same framework, different tools. |
