...
| `collectors/eodin-sdk.js` | `src/collectors/eodin-sdk.ts` | Eodin SDK (installs, DAU, funnel, cohort). **Wire shape exception:** `EodinSummaryRow` preserves upstream snake_case (`core_actions`, `paywall_views`) — this is an intentional deviation from the global camelCase rule to avoid a translation layer between the collector and the `sdk_events` table. **M3 constraint:** DDL for `sdk_events` MUST use snake_case column names. SSRF allowlist (`<EODIN_API_HOST>`) applied defense-in-depth even to `testHooks.baseUrl`. |
| `collectors/eodin-blog.js` | `src/collectors/eodin-blog.ts` | EodinBlogPublisher + Search Console + GA4 |
| `collectors/asomobile.js` | `src/collectors/asomobile.ts` | ASOMobile keyword API. SSRF allowlist (`api.asomobile.net`) applied. `AsoCompetitorInfo.description` is attacker-controllable — M4 skills must sanitize via `prompt-guard.ts`. |
