...
### Multi-turn skills (M6.7)

Until M6.7, every skill was one-shot: one message in, one `SkillResult`
out, thread ends. BrandSkill introduced a **multi-turn pattern** that
`core.ts` supports via a "Mode C" routing hook running before Mode A/B.
Flow state lives in the `brand_flows` SQLite table with a
`UNIQUE(user_id, thread_key)` index so at most one active flow exists
per (user, thread) pair. Key rules that any future multi-turn skill
must respect:

- Mode C matches on an active `brand_flows` row for `(userId, threadKey)`
  where `threadKey = ${channelId}:${threadId ?? "dm"}`. DMs share the
  `:dm` suffix so flows persist across DM messages.
- An explicit Mode A command always wins over an active flow — the
  operator can escape a stuck flow by re-issuing any registered skill
  command, which terminates the flow row first.
- The state machine (`src/brands/flow.ts`) is a pure reducer; all I/O
  and persistence lives in the skill (`src/skills/brand.ts`) so the
  conversation tree is unit-testable without DB or messenger mocks.
- Skills set `SkillResult.continuation` to stay active; they **never**
  get exposed as MCP tools (the 4-tool invariant still holds).
